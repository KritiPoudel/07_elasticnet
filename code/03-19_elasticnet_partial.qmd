...
```{r weather}
weather <- read_csv("../data/weather_monthsum.csv", show_col_types = F)
...
Before training, we need to perform some processing steps, like  
  ...
  - normalizing  
  - dropping NAs  
  - removing columns with single value  (zero variance - zv)
  - others?  
...
Based on R2, we would choose mixture = 0, penalty = 0.
...
R2 is higher in test set than train set.
RMSE is lower in test set than train set. 
Test set of data has better metrics than train set.
